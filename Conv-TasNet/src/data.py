...
class AudioDataset(data.Dataset):
    ...
    def __init__(self, json_dir, batch_size, sample_rate=8000, segment=4.0, cv_maxlen=8.0):
        ...
        infors=''
        with open(os.path.join(json_dir,'mix-target.json')) as f: 
            infors =json.load(f)
        mix_infos=[]
        s1_infos=[]
        for infor in infors:
            mix_infos.append([infor[0]])
            s1_infos.append([infor[1]])
        #mix_json = os.path.join(json_dir, 'mix.json')
        #s1_json = os.path.join(json_dir, 'target.json')
        ...
        if segment >= 0.0:
            ...
            while True:
                ...
                part_mix, part_s1 = [], []
                while num_segments < batch_size and end < len(mix_infos):
                    utt_len = int(mix_infos[end][1])
                    if utt_len >= segment_len:  # skip too short utt
                        ...
                        if num_segments > batch_size:
                            ...
                            if start == end: end += 1
                            break
                        part_mix.append(mix_infos[end])
                        part_s1.append(s1_infos[end])
                    end += 1
                if len(part_mix) > 0:
...
class EvalDataset(data.Dataset):
    ...
    def __init__(self, mix_dir, mix_json,  batch_size, sample_rate=8000):
        ...
        if mix_dir is not None:
            mix_infos= []
            wav_list = os.listdir(mix_dir)
            for wav in wav_list:
                mix_infos.append(os.path.join(mix_dir,wav))
        # sort it by #samples (impl bucket)
        # def sort(infos): return sorted(
        #     infos, key=lambda info: int(info[1]), reverse=True)
        # sorted_mix_infos = sort(mix_infos)
...
def load_mixtures_and_sources(batch):
    ...
    for mix_info, s1_info in zip(mix_infos, s1_infos):
        ...
        # merge s1 and s2
        s = s1.reshape(s1.shape[0],1) #T x C , C=1
